...
- **Markdown.** No line-width rule, and **paragraphs are written as a single line** rather than hard wrapped. Renderers and diff viewers soft-wrap, so a hard wrap buys nothing and costs a reflow every time a sentence is edited — and it makes a one-word change show up as a multi-line diff. Tables, code blocks and list items keep their own line structure. Files not otherwise being edited are left alone rather than reflowed for its own sake; `docs/requirements.md` and decisions `0001`–`0003` are still hard wrapped for that reason.
