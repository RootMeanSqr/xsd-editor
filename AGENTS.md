...
Seven are recorded so far: **Apache-2.0** in [`docs/decisions/0001-licensing.md`](docs/decisions/0001-licensing.md), **.NET with Avalonia** in [`docs/decisions/0002-technology-stack.md`](docs/decisions/0002-technology-stack.md), and **AvaloniaEdit for Text View** in [`docs/decisions/0003-text-view-editor-control.md`](docs/decisions/0003-text-view-editor-control.md), **build, corpus access and security tooling** in [`docs/decisions/0004-build-and-security-tooling.md`](docs/decisions/0004-build-and-security-tooling.md), **the syntax layer** in [`docs/decisions/0005-syntax-layer.md`](docs/decisions/0005-syntax-layer.md), **the move to Avalonia 12** in [`docs/decisions/0006-avalonia-12-migration.md`](docs/decisions/0006-avalonia-12-migration.md), and **ampersand escaping at validation rather than before parsing** in [`docs/decisions/0007-ampersand-escaping-at-validation.md`](docs/decisions/0007-ampersand-escaping-at-validation.md).
...
- Do not merge with an unresolved Critical or High finding from either security check (`XE-081`): composition scanning over dependencies, and static analysis over our own code. Each tool's severity words map onto those levels in [`docs/decisions/0004-build-and-security-tooling.md`](docs/decisions/0004-build-and-security-tooling.md); a new tool adds a row there rather than a new vocabulary. Medium and Low are triaged rather than gating **for composition scanning**; over our own code `TreatWarningsAsErrors` gates them too, which is deliberately stricter than `XE-081` requires. An exception to any of this is recorded in `docs/decisions/`, never left implicit.
