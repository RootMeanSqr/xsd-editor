...
- **Design View**:
  ...
  - **Changing an Element's Type**: Double-clicking the `Type | <typeName>` row on an element card opens a type picker and, on confirmation, rewrites the element's `type` attribute to the chosen type.
    ...
    - **Design View refreshes on confirmation.** The element's subtree is re-rendered against the new type. The previous type's structure no longer applies, so expansion state beneath that element is discarded; the canvas root and the expansion state of everything outside that subtree are unaffected.
