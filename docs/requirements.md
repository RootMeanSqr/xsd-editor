...
- **Design View**:
  ...
  - **Expansion State**: Expanded/collapsed is a property of the **object**, held per tab, not a property of a position on the canvas. Each tab keeps a marker for every object it has rendered; the schema file is never written to record it, and the state does not outlive the tab.
    ...
    - **Re-rooting always expands the root.** Re-rooting onto a type shows the immediate child elements of that type and of its base type (§2.2.1), whatever the stored markers said. The override is written to the markers rather than applied only to the render, so the root's state is thereafter what the user sees. Deeper levels follow their own markers.
    - **Recursion bound**: where a type occurs within its own expansion — directly or through a chain — the reoccurrence renders collapsed and is marked as recursive, regardless of its marker. Per-object state would otherwise expand a recursive type without limit. The bound is on the render, not the marker: the same type expands normally where it occurs outside its own subtree.
