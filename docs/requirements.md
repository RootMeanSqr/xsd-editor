...
- **Drag & Drop**: Elements and model groups are repositioned by dragging them directly on the canvas. Two moves are supported, and they are the same gesture distinguished only by where the drop lands.
  ...
  - **The reachable set is what is on screen.** A drop target must be a currently rendered, expanded box in the active tab. Drag and drop never scrolls, re-roots, or expands to reach a target the user cannot see, and it never spans tabs. Moving something to a type that is not on the canvas uses **Copy & Paste** (§2.3), which is also the way to *copy* rather than move — dragging has no copy variant, so the gesture has one meaning.
  - **Spring-loading**: hovering a dragged item over a collapsed box for a moment expands it so its group can be dropped into. This expansion is part of the render only and does not write an expansion marker (§2.2), on the same rule that navigation does not.
  ...
  - **Invalid drops are refused** rather than corrected, with the pointer showing a no-drop cue and no document change: dropping an item onto itself or into its own subtree, dropping into a SimpleType box, and dropping onto a recursion-bound `…` indicator (§2.2), which stands for structure that is not rendered and therefore has no position to drop into.
  - **Dropping into a base type box** (§2.2.1) edits the base type itself, not the derived type being viewed, and so changes every type derived from it. It is permitted — the base type box is a real type, not a read-only echo — but the editor first reports how many derived types the change reaches. The report informs; it does not block, on the same posture as **Delete** (§2.3).
