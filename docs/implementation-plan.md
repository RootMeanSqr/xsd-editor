...
**Ampersand handling** (`XE-070`) has no preprocessor at all, by [`0007`](decisions/0007-no-ampersand-preprocessor.md). Our lexer reads a raw `&` as ordinary text and has no parse failure to prevent, and the corpus turns out to contain no raw ampersand to be lenient about, so the syntax layer is simply strict: a raw `&` is reported wherever it appears, annotation text included, and a reference to a code point XML forbids is reported separately. The rule is pinned to `XmlReader`'s verdict by a parity test. Escaping moves to the editor's write path (`XE-088`), so a user typing an ampersand never creates one. An `&` already opening a valid reference is untouched throughout; the corpus would fail immediately across its 119 references otherwise. **The tree is therefore built over the original file bytes and round trip is an identity**, with no offset map in the fidelity path.
